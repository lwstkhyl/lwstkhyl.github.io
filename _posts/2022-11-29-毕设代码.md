...
# 不同细胞类型中的HERV_fraction分布
p_B <- ggplot(meta, aes(x = celltype, y = HERV_fraction, fill = celltype)) +
  geom_violin(scale = "width", trim = TRUE, color = "grey20", linewidth = 0.3) +
  geom_boxplot(width = 0.12, outlier.shape = NA, fill = "white", color = "black", linewidth = 0.3) +
  scale_fill_manual(values = celltype_cols) +
  scale_y_continuous(labels = label_percent(accuracy = 0.1)) +
  labs(x = NULL, y = "HERV_fraction") +
  theme_pub +
  theme(
    legend.position = "none",
    axis.text.x = element_text(angle = 45, hjust = 1)
...
class_counts <- GetAssayData(seu, assay = "HERV_class_scTE", slot = "counts")
df_C <- tibble(
  feature = rownames(class_counts),
  count = Matrix::rowSums(class_counts)
) %>%
  filter(count > 0) %>%
  arrange(desc(count)) %>%
  mutate(
    ...
    lab = ifelse(prop >= 0.05, percent(prop, accuracy = 0.1), "")
  )
class_cols <- c(
  "ERV1" = "#4DBBD5",
  "ERVK" = "#E64B35",
...
class_cols <- class_cols[names(class_cols) %in% as.character(df_C$feature)]
p_C <- ggplot(df_C, aes(x = "All cells", y = prop, fill = feature)) +
  geom_col(width = 0.7, color = "grey20", linewidth = 0.3) +
  geom_text(aes(y = ypos, label = lab), size = 3.5, color = "black") +
  scale_fill_manual(values = class_cols) +
  scale_y_continuous(labels = label_percent(accuracy = 1), expand = c(0, 0)) +
  labs(x = NULL, y = "Proportion of total HERV counts", fill = "Class") +
  theme_pub +
  theme(
    legend.position = "right"
  )
# HERV_subfamily_scTE在全体细胞中的组成比例
subfamily_counts <- GetAssayData(seu, assay = "HERV_subfamily_scTE", slot = "counts")
df_D0 <- tibble(
  feature = rownames(subfamily_counts),
  count = Matrix::rowSums(subfamily_counts)
) %>%
  filter(count > 0) %>%
  arrange(desc(count))
top_subfamily <- head(df_D0$feature, top_n_subfamily)
df_D <- df_D0 %>%
  mutate(feature = ifelse(feature %in% top_subfamily, feature, "Others")) %>%
  group_by(feature) %>%
  summarise(count = sum(count), .groups = "drop") %>%
  arrange(desc(count))
lev_D <- c(setdiff(df_D$feature, "Others"), "Others")
df_D <- df_D %>%
  mutate(
    prop = count / sum(count),
    feature = factor(feature, levels = lev_D),
    ypos = cumsum(prop) - prop / 2,
    lab = ifelse(prop >= 0.05, percent(prop, accuracy = 0.1), "")
  )
...
subfamily_cols <- c(subfamily_cols, "Others" = "#BDBDBD")
p_D <- ggplot(df_D, aes(x = "All cells", y = prop, fill = feature)) +
  geom_col(width = 0.7, color = "grey20", linewidth = 0.3) +
  geom_text(aes(y = ypos, label = lab), size = 3.2, color = "black") +
  scale_fill_manual(values = subfamily_cols) +
  scale_y_continuous(labels = label_percent(accuracy = 1), expand = c(0, 0)) +
  labs(x = NULL, y = "Proportion of total HERV counts", fill = "Subfamily") +
  theme_pub +
  theme(
    legend.position = "right",
...
plot_go_dot <- function(go_df_path_list, wide_or_long = c("wide", "long"), n_show = 6, cluster_keep = NA, module_name_list = NA, facet_by = "module") {
  ...
  if(wide_or_long == "wide"){
    ...
    go_df$group_lab <- go_df[[facet_by]]
...
wgcna_pheatmap_GO_plot <- function(celltype, go_df_path_list, herv_keep = NULL, cluster_order = NULL, highlight = NULL, module_keep = NULL, n_show = 6){
  ...
  p_herv <- as.ggplot(grobs_aligned[[1]]) |> pad_plot(l = 14, r = 10, t = 6, b = 6)
  p_module <- as.ggplot(grobs_aligned[[2]]) |> pad_plot(l = 14, r = 10, t = 6, b = 6)
  p_corr <- as.ggplot(grobs_aligned[[3]]) |> pad_plot(l = 14, r = 10, t = 6, b = 6)
  p_left <- plot_grid(
...
wgcna_pheatmap_GO_plot(
  celltype = "oligo",
  herv_keep = c("HERV3-int", "HERVH-int", "HERVL-int", "HERVK-int", "HERVK3-int", "HERVK9-int", "HERVK11-int", "HERVK13-int", "HERVK14-int", "HERVK22-int"),
...
func_axis_levels <- c(
  "gliogenesis_neural_support",
  "cell_cell_signaling",
  "cytokine_response",
  "synaptic_transmission",
  "synaptic_projection_program",
  "synaptic_program",
  "synaptic_homeostatic_program",
  "translation_OXPHOS",
  "myelination_mature_oligo",
  "lipid_membrane_signal_remodeling",
  "cytoskeleton_adhesion_remodeling",
  "innate_immune_pathogen_defense",
  "inflammation_TLR_cytokine",
  "protein_folding_stress",
  "STAT1_external_stimulus",
  "vascular_circulation_program",
  "replication_RNA_processing",
  "DNA_synthesis_cell_cycle"
)
func_axis_labels <- c(
  gliogenesis_neural_support = "Gliogenesis /\nNeural support",
  cell_cell_signaling = "Cell-cell\nsignaling",
  cytokine_response = "Cytokine\nresponse",
  synaptic_transmission = "Synaptic\ntransmission",
  synaptic_projection_program = "Synaptic projection /\norganization",
  synaptic_program = "Synaptic\nprogram",
  synaptic_homeostatic_program = "Synaptic /\nhomeostatic program",
  translation_OXPHOS = "Translation /\nOXPHOS",
  myelination_mature_oligo = "Myelination /\nMature oligo",
  lipid_membrane_signal_remodeling = "Lipid / membrane /\nsignal remodeling",
  cytoskeleton_adhesion_remodeling = "Cytoskeleton /\nadhesion remodeling",
  innate_immune_pathogen_defense = "Innate immune /\npathogen defense",
  inflammation_TLR_cytokine = "Inflammation /\nTLR-cytokine",
  protein_folding_stress = "Protein folding /\nstress",
  STAT1_external_stimulus = "STAT1 /\nexternal stimulus",
  vascular_circulation_program = "Vascular /\ncirculation",
  replication_RNA_processing = "Replication /\nRNA processing",
  DNA_synthesis_cell_cycle = "DNA synthesis /\ncell cycle"
)
...
df$main_label <- gsub("/", "\n", df$herv_label)
df$show_label <- as.character(df$show_label) %in% c("TRUE", "True", "true", "T", "1")
# 纵轴顺序
celltype_pos <- c(
  Mic   = 7.4,
  Oligo = 6.0,
...
has_tile_above <- mapply(
  function(ct, xpos) {
    above_ct <- above_map[as.character(ct)]
    if(is.na(above_ct)) return(FALSE)
    if(ct == "Oligo") return(T)
    any(df$celltype == above_ct & df$x == xpos)
  },
  df$celltype, df$x
)
cluster_offset <- 0.72
df <- df %>%
  mutate(
    cluster_dir = case_when(
      cluster_label == "" ~ NA_character_,
      has_tile_above ~ "down",
      TRUE ~ "up"
    ),
    cluster_y = case_when(
      cluster_label == "" ~ NA_real_,
      cluster_dir == "up" ~ y + cluster_offset,
      cluster_dir == "down" ~ y - cluster_offset
    )
  )
...
p <- ggplot(df, aes(x = x, y = y)) +
  ...
  geom_text(
    data = df_cluster,
    aes(x = x, y = cluster_y, label = cluster_label),
    inherit.aes = FALSE,
    size = 3.6,
    color = "black"
  ) +
  ...
  annotate(
    "text",
    x = max(df$x) + 1.3,
    y = mean(celltype_pos[c("Oligo", "Astro", "Excit")]),
    label = "Main\nfindings",
    angle = 0,
    fontface = "bold",
    size = 5
  ...
  annotate(
    "text",
    x = max(df$x) + 1.3,
    y = mean(celltype_pos[c("OPC", "Inhib", "Endo")]),
    label = "Supportive\nfindings",
    angle = 0,
    fontface = "bold",
    size = 5
...
ggsave(file.path(res_dir, "wgcna_summary.pdf"), p, width = 15, height = 8)
```
...
corr_mat_list <- wide2long(corr_mat_list, "cor")
fdr_mat_list <- wide2long(fdr_mat_list, "fdr")
p_corr <- plot_herv_gene_corr_df(seu_list, plot_specs)  # 4*3相关性线图
# 画图参数
base_size_main <- 12
title_size_main <- 14
