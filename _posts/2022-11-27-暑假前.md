...
write.csv(summary, file.path(res_dir, paste0("hERV_coverage_summary_", mode, ".csv")), row.names = FALSE)
ad_summary <- summary %>%
  filter(group == "AD") %>%
  select(
...
write.csv(ad_summary, file.path(res_dir, paste0("hERV_coverage_AD_focused_", mode, ".csv")), row.names = FALSE)
```

//todo 直接用igv看一下bam？
如果像incRNA就像基因可以看受哪些元件调控
