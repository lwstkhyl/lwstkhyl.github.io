...
- hERV专用注释[transcripts.gtf](https://media.githubusercontent.com/media/mlbendall/telescope_annotation_db/refs/heads/master/builds/HERV_rmsk.hg38.v2/transcripts.gtf?download=true)（来自stellarscope作者）
...
- 其它所需的metadata文件（包括两个数据集的SRR+GSM编号、样本信息）均在我的GitHub仓库[hERV_AD_snRNA-seq_analyze](https://github.com/lwstkhyl/hERV_AD_snRNA-seq_analyze)中获取
  ...
  - `family2group.xlsx`：根据stellarscope作者提供的hERV的[subfamily-group映射](https://github.com/mlbendall/telescope_annotation_db/blob/master/builds/HERV_rmsk.hg38.v2/families.tsv)整理得到
...
```r
run_de_one_celltype <- function(seu, ct, assay) {
  if(ct!="All"){
    obj <- subset(seu, subset = celltype == ct)
  }else{
    obj <- seu
...
write.csv(res_group, file.path(res_dir, "hERV_group_celltype.csv"))
```
...
calc_module_score("astro.rds", "Astro_HERV_family_0.1_4_50_0.2_gene_WGCNA.RData")
calc_module_score("oligo.rds", "Oligo_HERV_group_0.1_4_20_0.05_gene_WGCNA.RData")
calc_module_score("excit.rds", "Excit_HERV_family_0.1_4_50_0.2_gene_WGCNA.RData")
calc_module_score("inhib.rds", "Inhib_HERV_family_0.1_4_30_0.1_gene_WGCNA.RData")
calc_module_score("OPC.rds", "OPC_HERV_family_0.1_4_30_0.1_gene_WGCNA.RData")
calc_module_score("endo.rds", "Endo_HERV_family_0.1_4_30_0.1_gene_WGCNA.RData")
calc_module_score("mic.rds", "Mic_HERV_family_0.1_4_50_0.2_gene_WGCNA.RData")
```
...
run_go <- function(net_obj_name, target_modules = NA, ont = "BP", show_category = 15, n_hubs = 30) {
  ...
  modules_df <- subset(modules_df, module != "grey")
  if(length(target_modules)==1) if(is.na(target_modules)) target_modules <- as.character(unique(modules_df$module))
  # 目标模块的基因
...
run_go("Oligo_HERV_group_0.1_4_20_0.05_gene_WGCNA.RData", c("turquoise", "yellow", "blue"))
...
run_DEG_go("Oligo_HERV_group_0.1_4_20_0.05_gene_WGCNA.RData", c(
  "5" = "turquoise",
  "3" = "yellow",
  "9" = "blue"
))
