...
target_modules <- c("turquoise", "blue")
# 提取模块基因 + hub genes
modules_df <- GetModules(seu, wgcna_name = "gene_net")
hub_df <- GetHubGenes(seu, n_hubs = n_hubs, wgcna_name = "gene_net")
...
top_go_tbl <- lapply(target_modules, function(m) {
  ...
  data.frame(
    ...
    top_GO = if (nrow(go_df) > 0) go_df$Description[1] else NA,
    top_GO_padj = if (nrow(go_df) > 0) go_df$p.adjust[1] else NA,
    stringsAsFactors = FALSE
  )
}) %>% bind_rows()
write.csv(top_go_tbl, file.path(res_dir, "oligo_module_trait_GO_summary.csv"), row.names = FALSE)
```
...
modules_mixed <- GetModules(seu)
hub_mixed <- GetHubGenes(seu, n_hubs = 50)
```

![my_GSE157827_5_9](/upload/md-image/other/my_GSE157827_5_9.jpg){:width="800px" height="800px"}

```r
target_herv <- "HERV__HERVK-int"  # 目标hERV
# 按TOM相似度取邻居
TOM <- GetTOM(seu)
nbr <- sort(TOM[target_herv, ], decreasing = TRUE)
nbr <- nbr[names(nbr) != target_herv]
herv_neighbors <- data.frame(
  feature = names(nbr),
  TOM = as.numeric(nbr),
  stringsAsFactors = FALSE
)
herv_neighbors$type <- ifelse(grepl("^HERV__", herv_neighbors$feature), "hERV", "gene")
herv_gene_neighbors <- subset(herv_neighbors, type == "gene")  # 只保留gene
# 提取某个hERV节点的邻居基因
top_gene_neighbors <- head(herv_gene_neighbors, 100)
target_module <- subset(modules_mixed, gene_name == target_herv)$module[1]  # 这个hERV属于哪个module
# 同模块的gene邻居
same_module_genes <- subset(modules_mixed, module == target_module)$gene_name
top_same_module_gene_neighbors <- subset(top_gene_neighbors, feature %in% same_module_genes)
gene_list_for_go_mixed <- top_same_module_gene_neighbors$feature
```
