...
dev.off()
# 构建网络
seu <- ConstructNetwork(  # 运行时间20min，需要30G+内存
  seu,
...
# 计算module eigengenes
seu <- ModuleEigengenes(  # 运行时间20min，需要30G+内存
  seu,
...
# 目标hERV
target_trait <- "HERVK-int"
# 找显著相关模块
sig_modules <- rownames(mt$fdr)[mt$fdr[, target_trait] < 0.05]
mt$cor[sig_modules, target_trait, drop = FALSE]
modules_df <- GetModules(seu)
hub_df <- GetHubGenes(seu, n_hubs = 50)
# 取显著模块对应的hub genes
hub_sig <- subset(hub_df, module %in% sig_modules)
# 取某一个目标模块的全部基因
target_module <- sig_modules[1]
genes_in_target_module <- subset(
  modules_df,
  module == target_module & module != "grey"
)
gene_list_for_go <- genes_in_target_module$gene_name
```
...
seu[["RNA_HERV"]] <- CreateAssayseuect(data = mixed_data)
seu
