...
herv_gene_module_corr <- function(seu, herv_assay, target_herv){
  celltype_name <- identity_seu(seu)
  DefaultAssay(seu) <- "RNA"
  seu$AD_binary <- ifelse(seu$group == "AD", 1, 0)
  mat <- LayerData(seu, assay = herv_assay, layer = "data")[target_herv, , drop = FALSE]
...
plot_herv_gene_corr_panel <- function(seu_ct, herv_assay, herv_feature, gene_list, gene_assay, agg = c("mean", "sum"), is_donor = FALSE, donor_var = "orig.ident", ncol = length(herv_feature)) {
  ...
  for(gene in gene_list){
    for(herv in herv_feature){
      plots[[paste0(gene, "_", herv)]] <- plot_herv_gene_corr(seu_ct, herv_assay, herv, gene, agg, is_donor, donor_var)
    }
  }
...
plot_herv_gene_corr_panel(mic, "HERV", top_tlr8_loci, gene_list, "mean", TRUE)
```
...
plot_herv_gene_bubble <- function(cor_df, p_cutoff = 0.05) {
  ...
  ggplot(df, aes(x = signature, y = subfamily, size = abs(rho), color = rho)) +
    ...
    theme_bw() +
    labs(x = NULL, y = NULL, size = "|rho|", color = "rho")
}
plot_herv_gene_bubble(
