...
plot_list <- lapply(net_obj_name_list, function(net_obj_name){
  ...
  load(file.path(res_dir, net_obj_name))
  my_print("- calc corr")
  herv_gene_module_corr(seu, herv_locus_assay, target_herv_locus)
})
pdf(file.path(res_dir, "HERV_locus_subcluster_network_corr.pdf"), width = 32, height = 24)
print(wrap_plots(plot_list, ncol = 3))
dev.off()
```
...
run_herv_geneset_coexpr_one_celltype <- function(seu_ct, herv_assay, herv_features, gene_features, agg = c("sum", "mean"), is_donor = FALSE, donor_var = "orig.ident") {
  ct <- identity_seu(seu_ct)
  level_name <- ifelse(is_donor, "donor", "cell")
  res_name <- paste(ct, herv_assay, gene_features[1], agg, level_name, sep = " ")
  my_print(res_name, ":")
...
write.csv((mic_summary_HERVK_locus <- summary_HERVK_locus(mic)), file = file.path(res_dir, "mic_summary_HERVK_locus.csv"))
write.csv((all_summary_HERVK_locus <- summary_HERVK_locus(seu)), file = file.path(res_dir, "all_summary_HERVK_locus.csv"))
write.csv((mic2_summary_HERVK_locus <- summary_HERVK_locus(mic2)), file = file.path(res_dir, "mic2_summary_HERVK_locus.csv"))
# Mic
target_herv_locus <- mic_summary_HERVK_locus %>%
  filter(pct > 0.01) %>%
  pull(locus)
mic_res <- lapply(names(gene_panel), function(sig) {
  run_herv_geneset_coexpr_one_celltype(mic, herv_locus_assay, target_herv_locus, gene_panel[[sig]], agg = "mean", is_donor = T) %>%
    mutate(signature = sig, .before = 1)
}) %>% bind_rows()
write.csv(mic_res, file = file.path(res_dir, paste0("mic_geneset_hERV_locus_corr_donor.csv")))
mic_res <- lapply(names(gene_panel), function(sig) {
  run_herv_geneset_coexpr_one_celltype(mic, herv_locus_assay, target_herv_locus, gene_panel[[sig]], agg = "mean") %>%
    mutate(signature = sig, .before = 1)
}) %>% bind_rows()
write.csv(mic_res, file = file.path(res_dir, paste0("mic_geneset_hERV_locus_corr.csv")))
# 所有细胞
target_herv_locus <- all_summary_HERVK_locus %>%
  filter(pct > 0.01) %>%
  pull(locus)
all_res <- lapply(names(gene_panel), function(sig) {
  run_herv_geneset_coexpr_one_celltype(seu, herv_locus_assay, target_herv_locus, gene_panel[[sig]], agg = "mean", is_donor = T) %>%
    mutate(signature = sig, .before = 1)
}) %>% bind_rows()
write.csv(all_res, file = file.path(res_dir, paste0("all_geneset_hERV_locus_corr_donor.csv")))
all_res <- lapply(names(gene_panel), function(sig) {
  run_herv_geneset_coexpr_one_celltype(seu, herv_locus_assay, target_herv_locus, gene_panel[[sig]], agg = "mean") %>%
    mutate(signature = sig, .before = 1)
}) %>% bind_rows()
write.csv(all_res, file = file.path(res_dir, paste0("all_geneset_hERV_locus_corr.csv")))
# mic cluster2
target_herv_locus <- mic2_summary_HERVK_locus %>%
  filter(pct > 0.01) %>%
  pull(locus)
mic2_res <- lapply(names(gene_panel), function(sig) {
  run_herv_geneset_coexpr_one_celltype(mic2, herv_locus_assay, target_herv_locus, gene_panel[[sig]], agg = "mean", is_donor = T) %>%
    mutate(signature = sig, .before = 1)
}) %>% bind_rows()
write.csv(mic2_res, file = file.path(res_dir, paste0("mic2_geneset_hERV_locus_corr_donor.csv")))
mic2_res <- lapply(names(gene_panel), function(sig) {
  run_herv_geneset_coexpr_one_celltype(mic2, herv_locus_assay, target_herv_locus, gene_panel[[sig]], agg = "mean") %>%
    mutate(signature = sig, .before = 1)
}) %>% bind_rows()
write.csv(mic2_res, file = file.path(res_dir, paste0("mic2_geneset_hERV_locus_corr.csv")))
```
...
plot_herv_gene_corr <- function(seu_ct, herv_assay, herv_feature, gene, agg = c("mean", "sum"), is_donor = FALSE, donor_var = "orig.ident") {
  meta <- seu_ct@meta.data %>%
    mutate(log10_nCount_RNA = log10(nCount_RNA + 1))
  # hERV
  herv_mat <- GetAssayData(seu_ct, assay = herv_assay, slot = "data")
  if (!herv_feature %in% rownames(herv_mat)) {
    stop("hERV feature not found in assay.")
  }
  ...
  ggplot(df_plot, aes(x = x, y = y)) +
    ...
    theme_bw() +
    labs(
      title = paste0(gene_name, " ~ ", herv_feature),
      subtitle = paste0("rho=", rho_txt, ", p=", p_txt),
      x = herv_feature,
      y = gene_name
    )
}
# 多个基因/基因集（列表）与多个hERV（vector）
plot_herv_gene_corr_panel <- function(seu_ct, herv_assay, herv_feature, gene_list, gene_assay, agg = c("mean", "sum"), is_donor = FALSE, donor_var = "orig.ident", ncol = length(herv_feature)) {
  plots <- list()
  for(gene in gene_list){
    for(herv in herv_feature){
      plots[[paste0(gene[1], "_", herv)]] <- plot_herv_gene_corr(seu_ct, herv_assay, herv, gene, agg, is_donor, donor_var)
    }
  }
  ...
  pdf(
    ...
    width = 6 * ncol, 
    height = 6 * length(gene_list)
  )
  print(p)
  dev.off()
  return(plots)
}
gene_list <- list("TLR8", c("ISG15", "STAT1", "IFIT3"))
top_tlr8_loci <- c(
  "HML3-Xq13.3",
  "HML3-19p13.2c",
  "HML2-3q12.3",
...
plot_herv_gene_corr_panel(mic, "HERV", top_tlr8_loci, gene_list, agg = "mean", is_donor = TRUE)
```
