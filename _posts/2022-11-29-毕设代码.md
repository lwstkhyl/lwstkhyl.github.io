...
- 使用`SCTransform+integration`流程做数据预处理，尽量和论文对齐，尽量消除测序深度的影响
  - 因为服务器内存限制，后改为用`NormalizeData+harmony`来做
...
latent <- c("nCount_RNA", "percent_mito")
```
...
get_subCelltype <- function(ct, pc.use=1:30, harmony=F, resolution=0.3){
  ...
  if(harmony){
    ...
    sub <- RunUMAP(sub, reduction = "harmony", dims = pc.use, verbose = FALSE)
    sub <- FindNeighbors(sub, reduction = "harmony", dims = pc.use, verbose = FALSE)
  } else {
    sub <- RunUMAP(sub, dims = pc.use, verbose = FALSE)
    sub <- FindNeighbors(sub, dims = pc.use, verbose = FALSE)
  }
...
astro <- get_subCelltype("Astro", )
excit <- get_subCelltype("Excit", )
oligo <- get_subCelltype("Oligo", )
inhib <- get_subCelltype("Inhib", )
saveRDS(
  astro, 
