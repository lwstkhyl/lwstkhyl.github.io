...
- **另一个思路hERV在脑发育和衰老中整体调控变化，进而在脑部疾病中起作用**
- **需考虑多套数据**
...
```r
# 亚群级别分析
cluster_info <- atac@meta.data %>%
  as.data.frame() %>%
  count(cluster, celltype, group, sample_id, name = "n_cells")
cluster_summary <- cluster_info %>%
  ...
  pivot_wider(
    names_from = group,
    values_from = c(n_samples, n_cells),
    values_fill = 0
  )
counts_peak <- GetAssayData(atac, assay = "peaks", layer = "counts")
...
min_total_cells <- 100
clusters <- sort(unique(as.character(atac$cluster)))
all_res <- list()
summary_res <- list()
tested_peak_list <- list()
for (cl in clusters) {
  message("Running cluster: ", cl)
  md_cl <- atac@meta.data %>%
    as.data.frame() %>%
    rownames_to_column("cell") %>%
    filter(cluster == cl)
  if (nrow(md_cl) < min_total_cells) {
    message("Skip ", cl, ": total cells < ", min_total_cells)
    next
  }
  sample_keep <- md_cl %>%
    count(sample_id, group, name = "n_cells") %>%
  ...
  md_cl <- md_cl %>%
    filter(sample_id %in% sample_keep$sample_id)
  n_ad <- length(unique(md_cl$sample_id[md_cl$group == "AD"]))
  n_ctrl <- length(unique(md_cl$sample_id[md_cl$group == "Control"]))
  if (n_ad < min_samples_per_group | n_ctrl < min_samples_per_group) {
    message("Skip ", cl, ": too few samples after filtering")
    next
  }
  cells_use <- md_cl$cell
  X <- counts_peak[, cells_use, drop = FALSE]
  sample_factor <- factor(md_cl$sample_id)
  M <- Matrix::sparse.model.matrix(~ 0 + sample_factor)
  colnames(M) <- levels(sample_factor)
  pb_counts <- X %*% M
  sample_md <- md_cl %>%
  ...
  n_after <- nrow(y)
  if (n_after < 100) {
    message("Skip ", cl, ": too few peaks after filterByExpr")
    next
  }
  y <- calcNormFactors(y)
  design <- model.matrix(~ group, data = sample_md)
  ...
  res <- topTags(qlf, n = Inf)$table %>%
    ...
    mutate(
      cluster = cl,
      celltype = unique(md_cl$celltype)[1],
      cluster = unique(md_cl$cluster)[1],
      n_AD_samples = n_ad,
      n_Control_samples = n_ctrl,
      n_AD_cells = sum(md_cl$group == "AD"),
      n_Control_cells = sum(md_cl$group == "Control"),
      n_peaks_before_filter = n_before,
      n_peaks_after_filter = n_after,
      direction = case_when(
        FDR < 0.05 & logFC > 0 ~ "AD_more_open",
    ...
    select(
      cluster, celltype, peak,
      logFC, logCPM, F, PValue, FDR, direction,
      n_AD_samples, n_Control_samples,
      n_AD_cells, n_Control_cells,
      n_peaks_before_filter, n_peaks_after_filter
    )
  all_res[[cl]] <- res
  tested_peak_list[[cl]] <- data.frame(
    cluster = cl,
    peak = rownames(y$counts)
  )
  summary_res[[cl]] <- data.frame(
    cluster = cl,
    celltype = unique(md_cl$celltype)[1],
    n_AD_samples = n_ad,
    n_Control_samples = n_ctrl,
    ...
    n_FDR_0.1 = sum(res$FDR < 0.1),
    n_P_0.001 = sum(res$PValue < 0.001),
    n_AD_more_open_FDR_0.05 = sum(res$FDR < 0.05 & res$logFC > 0),
    n_AD_less_open_FDR_0.05 = sum(res$FDR < 0.05 & res$logFC < 0)
  )
}
all_res_df <- bind_rows(all_res)
summary_df <- bind_rows(summary_res)
tested_peak_df <- bind_rows(tested_peak_list)
wb <- createWorkbook()
addWorksheet(wb, "cluster_summary")
writeData(wb, "cluster_summary", cluster_summary)
addWorksheet(wb, "summary")
writeData(wb, "summary", summary_df)
addWorksheet(wb, "tested_peak")
writeData(wb, "tested_peak", tested_peak_df)
res_FDR_005 <- all_res_df %>%
  filter(FDR < 0.05) %>%
  arrange(cluster, FDR)
res_FDR_010 <- all_res_df %>%
  filter(FDR < 0.1) %>%
  arrange(cluster, FDR)
res_nominal_001 <- all_res_df %>%
  filter(PValue < 0.001) %>%
  arrange(cluster, PValue)
res_top200 <- all_res_df %>%
  group_by(cluster) %>%
  arrange(FDR, .by_group = TRUE) %>%
  slice_head(n = 200) %>%
  ungroup()
addWorksheet(wb, "top200")
writeData(wb, "top200", res_top200)
addWorksheet(wb, "sig_FDR_0.05")
writeData(wb, "sig_FDR_0.05", res_FDR_005)
addWorksheet(wb, "sig_FDR_0.1")
writeData(wb, "sig_FDR_0.1", res_FDR_010)
addWorksheet(wb, "sig_P_0.001")
writeData(wb, "sig_P_0.001", res_nominal_001)
saveWorkbook(
  wb,
  file = file.path(res_dir, "DA_peaks_edgeR_summary_cluster.xlsx"),
  overwrite = TRUE
)
for (ct in names(all_res)) {
  sheet_name <- gsub("[\\[\\]\\*\\?/\\\\:]", "_", ct)
...
saveWorkbook(
  wb,
  file = file.path(res_dir, "DA_peaks_edgeR_cluster.xlsx"),
  overwrite = TRUE
)
```
