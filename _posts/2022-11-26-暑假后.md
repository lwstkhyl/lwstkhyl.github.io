...
- 获取全长hERV转录本，参考[The landscape of hervRNAs transcribed from human endogenous retroviruses across human body sites](https://link.springer.com/article/10.1186/s13059-022-02804-w#Sec11)
- 选出值得研究的转录本
  - AD-NC差异表达的HERV
...
- 把候选转录本根据“它为什么表达变化、它和gene是什么关系”分成几类。*可能需要修改之前使用的`Retrotransposon:Gene Junction Reads Quantification`方法，或者找别的方法*
  - 嵌合转录本型：HERV被纳入宿主基因转录本中，作为alternative promoter、new exon、internal exon、3'UTR或terminator
...
- 挖掘上游调控因素
  ...
  - SNP层面：看是否有遗传变异调控HERV表达，即HERV-eQTL；再看这些SNP是否也和AD风险相关
- 与AD疾病作相关：
  - 如果找不到有基因型的数据，可以用别人做好的HERV-QTL+cortex AD GWAS summary做SMR：SNP → HERV表达 → AD风险，参考[2024 NC--Single-cell eQTL mapping of human endogenous retroviruses reveals cell type-specific genetic regulation in autoimmune diseases](https://www.nature.com/articles/s41467-025-62779-7)
...
要不要full length？主要snRNA-seq（或者加上snATAC-seq），是否把数据都整合到一起？如果发现某个可能作为promoter的hERV位点，再用长读长及其它特殊数据验证
...
conda create -n crispronbe python=3.10 tensorflow=2.10.0 biopython=1.79 viennarna=2.5.1 pandas=2.2.2 -y
cd ~
...
- **不改CRISPRon-BE内部结构，先用原CRISPRon-BE输出20-nt条件下的pred_eff和pred_freq，再额外训练一个小模型**：把原模型预测值、编辑位置、20-nt distance、18-nt distance等作为输入，预测18-nt条件下的编辑效率或窗口分布
  - 在原模型已经给出20-nt预测的基础上，加入R-loop distance后，能否更好地推断18-nt短sgRNA下的窗口收缩？
  - 还需要*18-nt真实编辑效率*
