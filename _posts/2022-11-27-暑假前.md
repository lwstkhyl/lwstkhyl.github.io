...
[预览](https://lwstkhyl.me/viewFile?fileName=AD_hERV_processed_junctions.xlsx&src=https%3A%2F%2Flwstkhyl.me%2Fapi%2Fdownload%2FATACtest%252Fres%252FAD_hERV_processed_junctions.xlsx) \| [下载](https://lwstkhyl.me/api/download/ATACtest%2Fres%2FAD_hERV_processed_junctions.xlsx?)
...
- 三个q显著结果其实是同一个SV区域：一个覆盖17q21.31-17q21.32附近多个HERV的DUP在AD中频率低于NC，并且对应HERV在转录层面表现为AD下调
  ...
  | cohort   | feature             | ID                          | sv_type | AF_AD | AF_NC | log2OR_AD_vs_NC_corr | fisher_p_AD_vs_NC | fisher_q_AD_vs_NC_in_matched | best_DE_sheet  | best_DE_celltype | best_DE_log2FC | best_DE_p_adj | direction_match_simplified |
  | -------- | ------------------- | --------------------------- | ------- | ----- | ----- | -------------------- | ----------------- | ---------------------------- | -------------- | ---------------- | -------------- | ------------- | -------------------------- |
  | illumina | HARLEQUIN-17q21.31  | chr17_34977736_39800003_DUP | DUP     | 0.125 | 0.215 | -0.931               | 0.000135          | 0.0291                       | GSE157827_MAST | All              | -0.0422        | 7.84e-81      | True                       |
  | illumina | HML4-17q21.31       | chr17_34977736_39800003_DUP | DUP     | 0.125 | 0.215 | -0.931               | 0.000135          | 0.0291                       | GSE157827_MAST | All              | -0.304         | 6.81e-31      | True                       |
  | illumina | HERVIP10FH-17q21.32 | chr17_34977736_39800003_DUP | DUP     | 0.125 | 0.215 | -0.931               | 0.000135          | 0.0291                       | GSE157827_MAST | All              | -0.13          | 6.79e-10      | True                       |

- 9个两个SV子队列均有重叠且all_MAST显著的HERV位点：
  | 位点                           | 原因                                                                                                                                 | 注意点                                                                     |
  | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- |
  | `HARLEQUIN-19p13.11`           | 两个队列都有SV<br>同一个SV+HERV在两个队列出现<br>转录总体AD上调<br>huada中DUP AD富集、illumina中DEL AD减少，方向上都可以解释表达升高 | SV很稀有，统计支持不强                                                     |
  | `PRIMA4-7q11.21b`              | 两个队列都有多个SV，且有同一SV复现<br>GSE157827和GSE174367中转录方向都偏AD上调<br>两个队列top DEL都是AD减少，方向上能解释AD表达升高  | SV频率差异未达到强统计显著                                                 |
  | `HML3-1p32.3`                  | 同一个SV+HERV在两个队列出现<br>Astro中AD下调较明显<br>该HERV与`ACOT11`同向exonic部分重叠，且相关性较高                               | illumina中SV类型较复杂，DUP/DEL都有，方向解释不如前两个清晰                |
  | `PRIMA41-19p12d`               | 转录在All/Inhib等细胞中偏AD上调<br>huada DUP AD富集、illumina DEL AD减少，方向上都可解释表达升高                                     | Excit中有相反方向记录，需按细胞类型拆开看                                  |
  | `HUERSP1-11q12.3`              | 转录下调<br>huada DEL AD富集、illumina DUP AD减少，方向上都能解释下调                                                                | huada中SV很稀有，illumina Fisher不显著                                     |
  | `HERVK11-1p13.3`               | Astro中下调<br>与`GSTM2`同向exonic full overlap，基因组位置值得看                                                                    | huada和illumina的top SV方向不完全一致，且all_MAST效应较小                  |
  | `HERVEA-19p13.11`              | 转录上调<br>两个队列都有SV                                                                                                           | huada DEL AD富集按直觉应导致表达下降，和转录方向冲突<br>illumina方向才匹配 |
  | `MER4B-7p14.3`、`MER4B-5q14.1` | 两个队列都有SV且all_MAST显著                                                                                                         | 转录效应较小，且不同数据集/细胞类型方向不完全稳定，优先级低一些            |
