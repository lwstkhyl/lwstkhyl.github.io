...
junction：在该pipeline中，最终的gene-hERV junction是“同一条read的一段比到gene exon、中间跳过一个intron、另一段比到hERV-derived exon或hERV区域”
- 在STAR中，STAR遇到一条RNA-seq read时会尝试
  - read前半段能比到基因组某处
  - read后半段能比到同一染色体下游某处
  - 两个片段方向、顺序合理
  - 中间距离符合intron范围
  ...
  如果这些条件成立，STAR就会把这条read作为spliced read比对，并记录一个splice junction
- pipeline负责找一端是gene一端是hERV的junction并计数
...
| te_side | 含义                                               | 简单理解                         |
| ------- | -------------------------------------------------- | -------------------------------- |
| `start` | junction的start端落在hERV/TE相关exon或hERV边界附近 | junction的左端是hERV端           |
| `end`   | junction的end端落在hERV/TE相关exon或hERV边界附近   | junction的右端是hERV端           |
| `both`  | start端和end端都检测到hERV/TE重叠                  | 两端都和hERV有关，解释时要更谨慎 |
...
[预览](https://lwstkhyl.me/viewFile?fileName=bulk_gene_hERV_junction.xlsx&src=https%3A%2F%2Flwstkhyl.me%2Fapi%2Fdownload%2FATACtest%252Fres%252Fbulk_gene_hERV_junction.xlsx) \| [下载](https://lwstkhyl.me/api/download/ATACtest%2Fres%2Fbulk_gene_hERV_junction.xlsx?)

| 文件                                           |                    行数 | 说明                             |
| ---------------------------------------------- | ----------------------: | -------------------------------- |
| `bulk_gene_hERV_junction_info`                 |           990条junction | 每条junction的注释信息           |
| `bulk_gene_hERV_junction_by_sample`            | 990条junction×191个样本 | 每条junction在各样本中的数量     |
| `bulk_gene_hERV_junction_group_summary`        |           990条junction | 每条junction按AD/Control汇总     |
| `bulk_gene_hERV_junction_region_group_summary` |                  1980行 | 每条junction按2个脑区(FC/CB)汇总 |
...
[预览](https://lwstkhyl.me/viewFile?fileName=gene_hERV_chimeric_candidates.xlsx&src=https%3A%2F%2Flwstkhyl.me%2Fapi%2Fdownload%2FATACtest%252Fres%252Fgene_hERV_chimeric_candidates.xlsx) \| [下载](https://lwstkhyl.me/api/download/ATACtest%2Fres%2Fgene_hERV_chimeric_candidates.xlsx?)
...
![暑假前_51](/upload/md-image/other/暑假前_51.jpg){:width="800px" height="800px"}
...
结合pyGenomeTracks画图分析后的候选位点：[预览](https://lwstkhyl.me/viewFile?fileName=gene_hERV_chimeric_candidates_pyGenomeTracks.xlsx&src=https%3A%2F%2Flwstkhyl.me%2Fapi%2Fdownload%2FATACtest%252Fres%252Fgene_hERV_chimeric_candidates_pyGenomeTracks.xlsx) \| [下载](https://lwstkhyl.me/api/download/ATACtest%2Fres%2Fgene_hERV_chimeric_candidates_pyGenomeTracks.xlsx?)

最核心的位点的pyGenomeTracks图：[预览](https://lwstkhyl.me/viewFile?fileName=merged_by_time.pdf&src=https%3A%2F%2Flwstkhyl.me%2Fapi%2Fdownload%2FATACtest%252Fres%252Fgene_hERV_browser%252Fmerged_by_time.pdf) \| [下载](https://lwstkhyl.me/api/download/ATACtest%2Fres%2Fgene_hERV_browser%2Fmerged_by_time.pdf?) \| [所有位点的图](https://lwstkhyl.me/api/download/ATACtest%2Fres%2Fgene_hERV_browser%2Fgene_hERV_browser.zip?)

![暑假前_53](/upload/md-image/other/暑假前_53.png){:width="800px" height="800px"}
- internal：HML5-2p13.2位于ZNF638基因跨度内部，目标gene span横跨整个区域，hERV处在gene body中间；bulk AD和bulk NC在整个区域都有明显覆盖，snRNA中AD和NC也都有大量峰

![暑假前_52](/upload/md-image/other/暑假前_52.png){:width="800px" height="800px"}
- internal：hERV位于gene注释内部或邻近区域，更适合解释为internal exon候选，而不是很典型的promoter，附近gene和hERV注释都比较密集，无法判断是否作为MRP55的promoter

![暑假前_54](/upload/md-image/other/暑假前_54.jpg){:width="800px" height="800px"}
- terminator：HERVIP10FH位于ZNF552目标gene的一端，紫色弧线从hERV附近连接到gene区域；hERV位点处也有reads峰分布，且junction连接hERV附近和ZNF552区域，说明ZNF552转录本可能延伸或剪接到HERVIP10FH区域

![暑假前_55](/upload/md-image/other/暑假前_55.jpg){:width="800px" height="800px"}
- promoter：hERV位于SCNN1D目标gene的5'端附近，紫色弧线也落在hERV边界附近，不过hERV与gene方向相反

为什么hERV在gene内却会被标为promoter？
- hERV虽然在gene span内部，但它是这个新isoform的5'端，所以可以被标为promoter_tss
  
  ```
  hERV/LTR promoter → hERV-derived first exon → downstream exon → downstream exon
  ```
- 现在还不能单独证明“RNA真的从hERV内部起始”

![暑假前_56](/upload/md-image/other/暑假前_56.jpg){:width="800px" height="800px"}


**存在一个问题：该pipeline在对hERV进行分类时未考虑链方向的影响**
- RepeatMasker中的hERV注释中的正负链是说这个基因组拷贝和“hERV家族共识序列”的相对方向，不一定等于真实转录方向
  
  ```
  hERV strand = +
  chr1: 1000 ---------------- 5000
        5'LTR -- internal -- 3'LTR
  hERV strand = -
  chr1: 1000 ---------------- 5000
        3'LTR -- internal -- 5'LTR
  ```
  
  或者说，是根据序列相似性判断“这个重复元件是正向还是反向插入”
- 在实际情况中，hERV内部或LTR里有反向启动活性、宿主gene读穿进入hERV（hERV只是被gene转录本包含进去）、邻近启动子或增强子激活局部转录都可能导致实际转录方向与标注的相反
- 如果要说明“hERV/LTR作为alternative promoter驱动gene isoform”：`HERV/LTR-derived 5' exon → gene exon2 → gene exon3 → ...`，这种时候就比较需要hERV与gene方向一致
- 在该pipeline中，只要hERV在gene的5'端，就会标为promoter候选
...
| 类型                             | 结构解释                                            | 需要的核心证据                                                                |
| -------------------------------- | --------------------------------------------------- | ----------------------------------------------------------------------------- |
| hERV-derived promoter/5' exon    | hERV/LTR提供新的5'端外显子，接到下游gene exon       | `promoter_tss/promoter`、同链、AD中有junction read、NC少或无、最好有5'端证据  |
| hERV-derived internal exon       | hERV作为中间外显子被纳入gene转录本                  | `internal`，最好同时有“上游gene exon→hERV”和“hERV→下游gene exon”两个junction  |
| hERV-derived terminal exon/3'UTR | gene转录延伸到hERV，hERV作为最后外显子或3'UTR样区域 | `terminator`、上游gene exon接到hERV、AD中更明显、最好有3'端/polyA或长读长证据 |

在这个pipeline的原论文[A mouse-specific retrotransposon drives a conserved Cdk2ap1 isoform essential for development](https://www.cell.com/cell/fulltext/S0092-8674(21)01104-1)中，作者发现一个mouse-specific MT2B2 retrotransposon作为promoter，驱动了一个新的Cdk2ap1 isoform(Cdk2ap1DN)。这个isoform不是普通Cdk2ap1转录本，而是从MT2B2附近起始，然后剪接到Cdk2ap1 exon2，因此跳过了canonical exon1，产生N端截短的蛋白
...
- 比较canonical isoform：`hERV-derived exon → gene exon2`- `canonical exon1 → gene exon2`
  
  如果AD中hERV→gene exon2上升，而canonical exon1→gene exon2不升甚至下降，就可能说明这个更可能是alternative promoter/isoform switching

//todo 嵌合转录本有没有新的方法（可以先用这套）

问题：该pipeline除了hERV和gene方向之外，还未考虑hERV和gene的重叠情况，即如果一个hERV和基因有重叠，那么hERV位于基因5'端时就会被归为promoter，但实际上该junction的first exon不仅属于hERV也属于基因，这样就无法区分这个hERV是否真正对基因转录产生了影响，还只是该基因正常转录 只不过恰好外显子也属于hERV
- 也许需要把hERV的exon信息也整合进来（原始hERV注释），这样是不是看的更清楚

**可变剪接差异表达**：找hERV作为外显子的有差异的可变剪接（`hERV-derived exon → gene exon2`- `canonical exon1 → gene exon2`）

```sh
conda create -n regtools regtools -y
conda create -n leafcutter_herv leafcutter -y
conda create -n r-tracklayer r-base r-data.table r-stringr r-openxlsx bioconductor-genomicranges bioconductor-rtracklayer -y  # conda activate r-harmony
```
...
library(data.table)
library(stringr)
res_dir <- "/public/home/GENE_proc/wth/GSE174367/0530test/chimeric_res_bulk/AS/"
metadata_file <- "/public/home/GENE_proc/wth/GSE174367/0530test/chimeric_res_bulk/metadata/bulk_sample_table.tsv"
bam_dir <- "/public/home/GENE_proc/wth/GSE174367/0530test/chimeric_res_bulk/star_2pass/"
meta <- fread(metadata_file)
meta <- meta[group %in% c("AD", "Control")]
meta[, sample_uid := GSM]
meta[, brain_region := ifelse(is.na(brain_region) | brain_region == "", "unknown", brain_region)]
meta[, AGE := as.numeric(AGE)]
meta[, pmi := as.numeric(pmi)]
meta[, pmi_missing := as.integer(is.na(pmi))]
meta[, pmi := fifelse(is.na(pmi), median(pmi, na.rm = TRUE), pmi)]
meta[, sex := ifelse(is.na(sex) | sex == "", "unknown", sex)]
bam <- data.table(bam = list.files(bam_dir, pattern = "\\.bam$", full.names = TRUE, recursive = TRUE))
bam[, GSM_in_bam := str_extract(bam, "GSM[0-9]+")]
bam[, SRR_in_bam := str_extract(bam, "SRR[0-9]+")]
meta[, bam := NA_character_]
for (i in seq_len(nrow(meta))) {
  hit <- bam[(!is.na(GSM_in_bam) & GSM_in_bam == meta$GSM[i]) | (!is.na(SRR_in_bam) & SRR_in_bam == meta$SRR[i]), bam]
  if (length(hit) > 0) meta$bam[i] <- hit[1]
}
meta <- unique(meta, by = "sample_uid")
fwrite(
  meta[, .(sample_uid, bam, group, brain_region, SRR, GSM, AGE, sex, pmi, pmi_missing)],
  file.path(res_dir, "leafcutter_bams.tsv"),
  sep = "\t"
)
min_n_per_group <- 4
for (br in sort(unique(meta$brain_region))) {
  x <- meta[brain_region == br]
  tab <- table(x$group)
  if (!all(c("AD", "Control") %in% names(tab))) next
  if (any(tab[c("AD", "Control")] < min_n_per_group)) {
    message("跳过脑区", br, "：AD或Control样本数少于", min_n_per_group)
    next
  }
  group_dt <- x[, .(sample_uid, group, sex, AGE, pmi, pmi_missing)]
  fwrite(group_dt, file.path(res_dir, paste0("group_", br, ".txt")), sep = "\t", col.names = FALSE)
  fwrite(x[, .(sample_uid, bam)], file.path(res_dir, paste0("bam_", br, ".tsv")), sep = "\t")
}
```

**2. 从BAM提取junction并运行LeafCutter**

```sh
cluster_py="/home1/wangtianhao/.conda/envs/leafcutter_herv/lib/python3.13/site-packages/leafcutter/clustering/leafcutter_cluster_regtools.py"
ds_cmd="leafcutter-ds"
res_dir="/public/home/GENE_proc/wth/GSE174367/0530test/chimeric_res_bulk/AS/"
threads=8
junc_dir="${res_dir}/regtools_junc"
lc_dir="${res_dir}/leafcutter"
mkdir -p "${junc_dir}" "${lc_dir}"
module load miniconda3/base
conda activate regtools
tail -n +2 "${res_dir}/leafcutter_bams.tsv" | cut -f1,2 | while IFS=$'\t' read -r sample_uid bam; do
  out="${junc_dir}/${sample_uid}.junc"
  if [[ ! -s "${out}" ]]; then
    echo "[regtools] ${sample_uid}"
...
conda activate leafcutter_herv
for group_file in "${res_dir}"/group_*.txt; do
  [[ -e "${group_file}" ]] || continue
  br=$(basename "${group_file}" | sed 's/^group_//' | sed 's/\.txt$//')
  out_region="${lc_dir}/${br}"
  mkdir -p "${out_region}"
  prefix="${out_region}/LC_${br}_AD_vs_Control"
  sig_file="${prefix}_cluster_significance.txt"
  eff_file="${prefix}_effect_sizes.txt"
  awk -v junc_dir="${junc_dir}" 'BEGIN{FS="\t"}{print junc_dir"/"$1".junc"}' "${group_file}" > "${res_dir}/junc_list_${br}.txt"
  counts_file="${out_region}/LC_${br}_perind_numers.counts.gz"
  if [[ ! -s "${counts_file}" ]]; then
    echo "[LeafCutter cluster] ${br}"
    python "${cluster_py}" \
      -j "${res_dir}/junc_list_${br}.txt" \
      -m 50 \
      -l 500000 \
      -o "LC_${br}" \
      -r "${out_region}"
    ...
    echo "[skip cluster] ${br}已有${counts_file}"
  fi
  echo "[LeafCutter Python differential splicing] ${br}"
...
**3. 把LeafCutter结果和gene-hERV junction整合**
- 读取LeafCutter cluster counts
- 读取之前pipeline的bulk_gene_hERV_junction_info
- 在LeafCutter cluster中找到hERV junction
- 找同一cluster里共享gene exon边界的canonical junction
- 计算hERV junction usage
- 标注hERV端是否落在LTR/internal
- 标注hERV区域是否和同一gene已知exon重叠
...
library(openxlsx)
res_dir <- "/public/home/GENE_proc/wth/GSE174367/0530test/chimeric_res_bulk/AS"
chimeric_dir <- "/public/home/GENE_proc/wth/GSE174367/0530test/chimeric_res_bulk"
metadata_file <- "/public/home/GENE_proc/wth/GSE174367/0530test/chimeric_res_bulk/metadata/bulk_sample_table.tsv"
herv_info_file <- "/public/home/GENE_proc/wth/GSE174367/0530test/chimeric_res_bulk/bulk_gene_hERV_junction_info.tsv"
gencode_gtf <- "/public/home/wangtianhao/Desktop/STAR_ref/gencode.v49.annotation.gtf"
herv_gtf <- "/public/home/GENE_proc/wth/metadata/hERV/hERV.gtf"
out_dir <- file.path(res_dir, "hERV_leafcutter_annotated")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
find_file <- function(x) {
  x <- x[file.exists(x)]
...
read_any_table <- function(path, sheet = 1) {
  if (grepl("\\.xlsx$", path, ignore.case = TRUE)) {
    x <- as.data.table(read.xlsx(path, sheet = sheet))
...
parse_info <- function(x) {
  ...
  data.table(
    info = x,
    chr = p[[1]],
    junc_start = as.integer(p[[2]]),
    junc_end = as.integer(p[[3]])
  )
}
gene_id0_one <- function(x) {
  x <- as.character(x)
  x <- strsplit(x, "/", fixed = TRUE)[[1]]
  x <- sub("\\..*", "", x)
  x[!is.na(x) & x != ""]
}
extract_info_from_intron <- function(x) {
  x <- as.character(x)
  sub("^([^:]+:[0-9]+:[0-9]+).*", "\\1", x)
}
extract_cluster_from_intron <- function(x) {
  x <- as.character(x)
...
get_col_value <- function(dt, col, default = NA) {
  if (is.null(dt) || nrow(dt) == 0 || !(col %in% names(dt))) return(default)
  v <- dt[[col]][1]
...
safe_fisher <- function(a, b, c, d) {
  if (sum(a, b, c, d, na.rm = TRUE) == 0) return(NA_real_)
  tryCatch(
    fisher.test(matrix(c(a, b, c, d), nrow = 2, byrow = TRUE))$p.value,
    error = function(e) NA_real_
  )
}
safe_wilcox <- function(dt) {
  dt2 <- dt[total > 0 & !is.na(usage)]
  if (nrow(dt2) < 4) return(NA_real_)
  if (length(unique(dt2$group)) < 2) return(NA_real_)
  tryCatch(
    wilcox.test(usage ~ group, data = dt2)$p.value,
    error = function(e) NA_real_
  )
}
parse_te_interval <- function(x) {
  x <- as.character(x)
  if (is.na(x) || x == "") return(NULL)
  if (grepl("^chr[^_]+_[0-9]+_[0-9]+$", x)) {
    z <- strsplit(x, "_")[[1]]
    return(data.table(chr = z[1], start = as.integer(z[2]), end = as.integer(z[3])))
  }
  if (grepl("^chr[^:]+:[0-9]+-[0-9]+$", x)) {
    z1 <- strsplit(x, ":", fixed = TRUE)[[1]]
    z2 <- strsplit(z1[2], "-", fixed = TRUE)[[1]]
    return(data.table(chr = z1[1], start = as.integer(z2[1]), end = as.integer(z2[2])))
  }
  NULL
}
# 读取metadata
...
meta[, sex := ifelse(is.na(sex) | sex == "", "unknown", sex)]
# 读取gene-hERV junction注释
herv_info <- read_any_table(herv_info_file)
herv_info[, info := sub("\\|.*", "", as.character(info))]
pos <- parse_info(herv_info$info)
herv_info[, chr := pos$chr]
herv_info[, junc_start := pos$junc_start]
herv_info[, junc_end := pos$junc_end]
herv_info <- herv_info[te_side %in% c("start", "end")]
herv_info[, gene_id0_list := lapply(gene_name, gene_id0_one)]
herv_info[, shared_gene_boundary := fifelse(te_side == "start", junc_end, junc_start)]
herv_info[, herv_boundary := fifelse(te_side == "start", junc_start, junc_end)]
herv_info[, hERV_te_info := fifelse(te_side == "start", as.character(start_te_info), as.character(end_te_info))]
herv_info <- unique(herv_info, by = "info")
all_herv_infos <- unique(herv_info$info)
message("[loaded] gene-hERV junctions: ", nrow(herv_info))
# 读取GENCODE exon，用于判断hERV端是否和同一gene已知exon重叠
message("[load] GENCODE exon")
gtf <- import(gencode_gtf)
gtf_exon <- gtf[gtf$type == "exon"]
gene_exon_gr <- GRanges(
  seqnames = as.character(seqnames(gtf_exon)),
  ranges = ranges(gtf_exon),
  strand = strand(gtf_exon),
  gene_id0 = sub("\\..*", "", as.character(mcols(gtf_exon)$gene_id)),
  gene_id = as.character(mcols(gtf_exon)$gene_id),
  gene_name_symbol = as.character(mcols(gtf_exon)$gene_name)
)
# 读取hERV.gtf结构片段，用于判断hERV端落在LTR/internal等区域
message("[load] hERV GTF")
hgtf <- import(herv_gtf)
herv_part <- hgtf[hgtf$type == "exon"]
herv_part_gr <- GRanges(
  seqnames = as.character(seqnames(herv_part)),
  ranges = ranges(herv_part),
  strand = strand(herv_part),
  geneRegion = as.character(mcols(herv_part)$geneRegion),
  repName = as.character(mcols(herv_part)$repName),
  repFamily = as.character(mcols(herv_part)$repFamily),
  repClass = as.character(mcols(herv_part)$repClass),
  locus = as.character(mcols(herv_part)$locus)
)
annotate_herv_part <- function(chr, boundary, tol = 10) {
  if (is.na(chr) || is.na(boundary)) {
    return(data.table(
      hERV_part_region = NA_character_,
      hERV_part_repName = NA_character_,
      hERV_part_repFamily = NA_character_,
      hERV_part_repClass = NA_character_,
      hERV_part_locus = NA_character_
    ))
  }
  gr <- GRanges(seqnames = chr, ranges = IRanges(boundary - tol, boundary + tol))
  hit <- findOverlaps(gr, herv_part_gr, ignore.strand = TRUE)
  if (length(hit) == 0) {
    return(data.table(
      hERV_part_region = NA_character_,
      hERV_part_repName = NA_character_,
  ...
  hp <- herv_part_gr[subjectHits(hit)]
  data.table(
    hERV_part_region = paste(unique(mcols(hp)$geneRegion), collapse = ";"),
    hERV_part_repName = paste(unique(mcols(hp)$repName), collapse = ";"),
    hERV_part_repFamily = paste(unique(mcols(hp)$repFamily), collapse = ";"),
    hERV_part_repClass = paste(unique(mcols(hp)$repClass), collapse = ";"),
    hERV_part_locus = paste(unique(mcols(hp)$locus), collapse = ";")
  )
}
same_gene_exon_overlap_bp <- function(te_info, gene_ids0) {
  z <- parse_te_interval(te_info)
  if (is.null(z)) return(NA_integer_)
  gr <- GRanges(seqnames = z$chr, ranges = IRanges(z$start, z$end))
  hit <- findOverlaps(gr, gene_exon_gr, ignore.strand = TRUE)
  if (length(hit) == 0) return(0L)
  ex <- gene_exon_gr[subjectHits(hit)]
  keep <- mcols(ex)$gene_id0 %in% gene_ids0
  if (!any(keep)) return(0L)
  ex <- ex[keep]
  gr_rep <- rep(gr, length(ex))
  sum(width(pintersect(gr_rep, ex)))
}
# 读取LeafCutter counts/effect/cluster结果
read_counts_file <- function(counts_file) {
  header_line <- readLines(gzfile(counts_file), n = 1)
  sample_names <- strsplit(header_line, "[\t ]+")[[1]]
  sample_names <- sample_names[sample_names != ""]
  sample_names <- make.unique(clean_sample_name(sample_names))
  x <- fread(
    cmd = paste("gzip -dc", shQuote(counts_file), "| tail -n +2"),
    header = FALSE,
    sep = " ",
    fill = TRUE,
    data.table = TRUE
  )
  setnames(x, names(x), c("intron", sample_names))
  x[, intron := as.character(x[[1]])]
  x[, info := extract_info_from_intron(x[[1]])]
  p <- tstrsplit(x$info, ":", fixed = TRUE)
  x[, chr := p[[1]]]
  x[, intron_start := as.integer(p[[2]])]
  x[, intron_end := as.integer(p[[3]])]
  x[, leafcutter_cluster := extract_cluster_from_intron(x[[1]])]
  meta_cols <- c("intron", "info", "chr", "intron_start", "intron_end", "leafcutter_cluster")
  count_cols <- setdiff(names(x), meta_cols)
  x[, (count_cols) := lapply(.SD, function(v) as.numeric(as.character(v))), .SDcols = count_cols]
  x[is.na(x)] <- 0
  x
}
read_effect_file <- function(effect_file) {
  if (!file.exists(effect_file)) return(NULL)
  x <- fread(effect_file)
  if (!"intron" %in% names(x)) {
    setnames(x, 1, "intron")
  }
  x[, intron := as.character(intron)]
  x[, info := extract_info_from_intron(intron)]
  x[, leafcutter_cluster := extract_cluster_from_intron(intron)]
  x
}
read_cluster_file <- function(cluster_file) {
  if (!file.exists(cluster_file)) return(NULL)
  x <- fread(cluster_file)
  if (!"cluster" %in% names(x)) {
    setnames(x, 1, "cluster")
  }
  if ("p.adjust" %in% names(x)) setnames(x, "p.adjust", "leafcutter_cluster_q")
  if ("p" %in% names(x)) setnames(x, "p", "leafcutter_cluster_p")
  if ("status" %in% names(x)) setnames(x, "status", "leafcutter_status")
  x[, cluster := as.character(cluster)]
  x
}
counts_files <- list.files(
  file.path(res_dir, "leafcutter"),
  pattern = "perind_numers\\.counts\\.gz$",
  full.names = TRUE,
  recursive = TRUE
)
message("[found] perind_numers counts files: ", length(counts_files))
print(counts_files)
# 每个脑区整合
process_one_region <- function(counts_file) {
  br <- basename(dirname(counts_file))
  message("[process] brain_region = ", br)
  message("- reading counts file:", counts_file)
  counts <- read_counts_file(counts_file)
  effect_file <- list.files(
    dirname(counts_file),
    pattern = "AD_vs_Control_effect_sizes\\.txt$",
    full.names = TRUE
  )
  effect_file <- if (length(effect_file) > 0) effect_file[1] else NA_character_
  cluster_file <- list.files(
    dirname(counts_file),
    pattern = "AD_vs_Control_cluster_significance\\.txt$",
    full.names = TRUE
  )
  cluster_file <- if (length(cluster_file) > 0) cluster_file[1] else NA_character_
  message("- reading effect file")
  effects <- if (!is.na(effect_file)) read_effect_file(effect_file) else NULL
  message("- reading cluster file")
  clusters <- if (!is.na(cluster_file)) read_cluster_file(cluster_file) else NULL
  if (is.null(effects)) {
    message("[warning] 没有找到effect_sizes文件：", br, "；该脑区仍会用counts计算usage，但LeafCutter效应量和p值为NA。")
  }
  if (is.null(clusters)) {
    message("[warning] 没有找到cluster_significance文件：", br, "；该脑区LeafCutter cluster p/q为NA。")
  }
  meta_br <- meta[brain_region == br]
  sample_cols <- intersect(meta_br$sample_uid, names(counts))
  meta_br <- meta_br[sample_uid %in% sample_cols]
  if (length(sample_cols) < 4) {
    message("[skip] ", br, "：可匹配样本数少于4")
    return(NULL)
  }
  if (!all(c("AD", "Control") %in% unique(meta_br$group))) {
    message("[skip] ", br, "：缺少AD或Control")
    return(NULL)
  }
  hinfo_region <- herv_info[chr %in% unique(counts$chr)]
  match_herv_lc_infos <- function(counts, hinfo_region, tol = 3) {
    out <- vector("list", nrow(hinfo_region))
    for (ii in seq_len(nrow(hinfo_region))) {
      hh2 <- hinfo_region[ii]
      out[[ii]] <- counts[
        chr == hh2$chr &
          abs(intron_start - hh2$junc_start) <= tol &
          abs(intron_end - hh2$junc_end) <= tol,
        info
      ]
    }
    unique(unlist(out))
  }
  all_herv_lc_infos <- match_herv_lc_infos(counts, hinfo_region, tol = 3)
  out <- vector("list", nrow(hinfo_region))
  k <- 1
  for (i in seq_len(nrow(hinfo_region))) {
    hh <- hinfo_region[i]
    hinfo <- hh$info
    hit <- counts[info == hinfo]
    if (nrow(hit) == 0) {
      hit <- counts[
        chr == hh$chr &
          abs(intron_start - hh$junc_start) <= 3 &
          abs(intron_end - hh$junc_end) <= 3
      ]
    }
    if (nrow(hit) == 0) next
    for (j in seq_len(nrow(hit))) {
      hrow <- hit[j]
      clu <- hrow$leafcutter_cluster
      clu_dt <- counts[leafcutter_cluster == clu]
      current_herv_lc_info <- hrow$info
      canonical <- clu_dt[
        !(info %in% unique(c(all_herv_lc_infos, current_herv_lc_info))) &
          (abs(intron_start - hh$shared_gene_boundary) <= 3 |
            abs(intron_end - hh$shared_gene_boundary) <= 3)
      ]
      hvec <- as.numeric(hrow[, ..sample_cols])
      cvec <- if (nrow(canonical) == 0) {
        rep(0, length(sample_cols))
      } else {
        colSums(as.matrix(canonical[, ..sample_cols]))
      }
      dt <- data.table(sample_uid = sample_cols, hERV = hvec, canonical = cvec)
      dt <- merge(dt, meta_br[, .(sample_uid, group, AGE, sex, pmi)], by = "sample_uid", all.x = TRUE)
      dt[, total := hERV + canonical]
      dt[, usage := fifelse(total > 0, hERV / total, NA_real_)]
      ad <- dt[group == "AD"]
      ct <- dt[group == "Control"]
      sum_h_ad <- sum(ad$hERV, na.rm = TRUE)
      sum_c_ad <- sum(ad$canonical, na.rm = TRUE)
      sum_h_ct <- sum(ct$hERV, na.rm = TRUE)
      sum_c_ct <- sum(ct$canonical, na.rm = TRUE)
      fisher_p <- safe_fisher(sum_h_ad, sum_c_ad, sum_h_ct, sum_c_ct)
      wilcox_p <- safe_wilcox(dt)
      eff_hit <- NULL
      if (!is.null(effects)) {
        eff_hit <- effects[info == hrow$info]
        if (nrow(eff_hit) == 0) eff_hit <- effects[info == hinfo]
        if (nrow(eff_hit) > 0) eff_hit <- eff_hit[1]
      }
      clu_hit <- NULL
      if (!is.null(clusters)) {
        clu_hit <- clusters[cluster == clu]
        if (nrow(clu_hit) > 0) clu_hit <- clu_hit[1]
      }
      hp <- annotate_herv_part(hh$chr, hh$herv_boundary)
      same_bp <- same_gene_exon_overlap_bp(hh$hERV_te_info, hh$gene_id0_list[[1]])
      canonical_infos <- canonical$info
      canonical_introns <- canonical$intron
      out[[k]] <- cbind(
        data.table(
          brain_region = br,
          leafcutter_cluster = clu,
          hERV_junction = hrow$info,
          hERV_intron_id = hrow$intron,
          canonical_junctions = paste(unique(canonical_infos), collapse = ";"),
          canonical_introns = paste(unique(canonical_introns), collapse = ";"),
          n_canonical_junctions = length(unique(canonical_infos)),
          gene_name = hh$gene_name,
          hERV_name = hh$hERV_name,
          junction_type = hh$junction_type,
          te_side = hh$te_side,
          junc_start = hh$junc_start,
          junc_end = hh$junc_end,
          shared_gene_boundary = hh$shared_gene_boundary,
          herv_boundary = hh$herv_boundary,
          hERV_te_info = hh$hERV_te_info,
          sum_hERV_AD = sum_h_ad,
          sum_canonical_AD = sum_c_ad,
          sum_hERV_Control = sum_h_ct,
          sum_canonical_Control = sum_c_ct,
          n_AD_hERV_detected = sum(ad$hERV > 0, na.rm = TRUE),
          n_Control_hERV_detected = sum(ct$hERV > 0, na.rm = TRUE),
          n_AD_total_detected = sum(ad$total > 0, na.rm = TRUE),
          n_Control_total_detected = sum(ct$total > 0, na.rm = TRUE),
          mean_usage_AD = safe_mean(ad[total > 0]$usage),
          mean_usage_Control = safe_mean(ct[total > 0]$usage),
          delta_usage_AD_vs_Control = safe_mean(ad[total > 0]$usage) - safe_mean(ct[total > 0]$usage),
          log2OR_usage_AD_vs_Control = log2(((sum_h_ad + 0.5) / (sum_c_ad + 0.5)) / ((sum_h_ct + 0.5) / (sum_c_ct + 0.5))),
          simple_fisher_p = fisher_p,
          simple_wilcox_p = wilcox_p,
          leafcutter_logef_AD = get_col_value(eff_hit, "logef_AD"),
          leafcutter_psi_Control = get_col_value(eff_hit, "psi_Control"),
          leafcutter_psi_AD = get_col_value(eff_hit, "psi_AD"),
          leafcutter_deltapsi_AD = get_col_value(eff_hit, "deltapsi_AD"),
          leafcutter_cluster_status = get_col_value(clu_hit, "leafcutter_status"),
          leafcutter_cluster_loglr = get_col_value(clu_hit, "loglr"),
          leafcutter_cluster_df = get_col_value(clu_hit, "df"),
          leafcutter_cluster_p = get_col_value(clu_hit, "leafcutter_cluster_p"),
          leafcutter_cluster_q = get_col_value(clu_hit, "leafcutter_cluster_q"),
          leafcutter_cluster_genes = get_col_value(clu_hit, "genes"),
          same_gene_exon_overlap_bp = same_bp,
          modeled_by_leafcutter_ds = !is.null(eff_hit) && nrow(eff_hit) > 0
        ),
        hp
      )
      k <- k + 1
...
all_res <- rbindlist(lapply(counts_files, process_one_region), fill = TRUE)
# 结果整理和候选分级
all_res[, simple_fisher_q := p.adjust(simple_fisher_p, method = "BH")]
all_res[, simple_wilcox_q := p.adjust(simple_wilcox_p, method = "BH")]
all_res[, has_canonical := n_canonical_junctions > 0]
all_res[, AD_hERV_only := n_AD_hERV_detected >= 2 & n_Control_hERV_detected == 0]
all_res[, novel_hERV_exon_like := !is.na(same_gene_exon_overlap_bp) & same_gene_exon_overlap_bp == 0]
all_res[, overlaps_same_gene_exon := !is.na(same_gene_exon_overlap_bp) & same_gene_exon_overlap_bp > 0]
all_res[, leafcutter_AD_usage_up := !is.na(leafcutter_deltapsi_AD) & leafcutter_deltapsi_AD > 0]
all_res[, manual_AD_usage_up := !is.na(delta_usage_AD_vs_Control) & delta_usage_AD_vs_Control > 0]
all_res[, candidate_level := fifelse(
  modeled_by_leafcutter_ds & has_canonical & leafcutter_AD_usage_up & leafcutter_cluster_q < 0.1 & novel_hERV_exon_like,
  "A_leafcutter_q0.1_AD_hERV_usage_up_novel_like",
  fifelse(
    modeled_by_leafcutter_ds & has_canonical & leafcutter_AD_usage_up & leafcutter_cluster_p < 0.05 & novel_hERV_exon_like,
    "B_leafcutter_nominal_AD_hERV_usage_up_novel_like",
    fifelse(
      modeled_by_leafcutter_ds & has_canonical & leafcutter_AD_usage_up & overlaps_same_gene_exon,
      "C_AD_usage_up_but_overlaps_same_gene_exon",
      fifelse(
        !modeled_by_leafcutter_ds,
        "D_hERV_junction_not_modeled_by_leafcutter_ds",
        fifelse(
          !has_canonical,
          "E_no_canonical_competitor_in_cluster",
          "F_other"
        )
      )
    )
...
setorder(
  ...
  candidate_level,
  brain_region,
  leafcutter_cluster_q,
  leafcutter_cluster_p,
  -leafcutter_deltapsi_AD,
  -delta_usage_AD_vs_Control
)
# 保存数据
all_tsv <- file.path(out_dir, "hERV_leafcutter_AS_events_all.tsv")
candidate_tsv <- file.path(out_dir, "hERV_leafcutter_AS_events_candidates.tsv")
xlsx_file <- file.path(out_dir, "hERV_leafcutter_AS_events.xlsx")
summary_tsv <- file.path(out_dir, "hERV_leafcutter_AS_summary.tsv")
fwrite(all_res, all_tsv, sep = "\t")
candidate_res <- all_res[candidate_level %in% c(
  "A_leafcutter_q0.1_AD_hERV_usage_up_novel_like",
  "B_leafcutter_nominal_AD_hERV_usage_up_novel_like",
  "C_AD_usage_up_but_overlaps_same_gene_exon"
)]
fwrite(candidate_res, candidate_tsv, sep = "\t")
summary_res <- all_res[, .N, by = .(brain_region, candidate_level)]
setorder(summary_res, brain_region, candidate_level)
fwrite(summary_res, summary_tsv, sep = "\t")
wb <- createWorkbook()
add_sheet <- function(wb, sheet_name, dt) {
  addWorksheet(wb, sheet_name)
  writeData(wb, sheet_name, dt)
  freezePane(wb, sheet_name, firstRow = TRUE)
  setColWidths(wb, sheet_name, cols = 1:ncol(dt), widths = "auto")
}
add_sheet(wb, "summary", summary_res)
add_sheet(wb, "all_events", all_res)
add_sheet(wb, "candidates", candidate_res)
add_sheet(wb, "A_strong", all_res[candidate_level == "A_leafcutter_q0.1_AD_hERV_usage_up_novel_like"])
add_sheet(wb, "B_nominal", all_res[candidate_level == "B_leafcutter_nominal_AD_hERV_usage_up_novel_like"])
add_sheet(wb, "C_overlap_same_gene_exon", all_res[candidate_level == "C_AD_usage_up_but_overlaps_same_gene_exon"])
add_sheet(wb, "AD_hERV_only", all_res[AD_hERV_only == TRUE])
add_sheet(wb, "not_modeled_by_DS", all_res[candidate_level == "D_hERV_junction_not_modeled_by_leafcutter_ds"])
add_sheet(wb, "no_canonical", all_res[candidate_level == "E_no_canonical_competitor_in_cluster"])
saveWorkbook(wb, xlsx_file, overwrite = TRUE)
```

[预览](https://lwstkhyl.me/viewFile?fileName=hERV_leafcutter_AS_events_new.xlsx&src=https%3A%2F%2Flwstkhyl.me%2Fapi%2Fdownload%2FATACtest%252Fres%252FhERV_leafcutter_AS_events_new.xlsx) \| [下载](https://lwstkhyl.me/api/download/ATACtest%2Fres%2FhERV_leafcutter_AS_events_new.xlsx?)

可以找到“hERV与gene外显子重叠的外显子 在AD中使用比例升高”的情况，但是没找到“hERV未与基因重叠的外显子作为剪接junction”的情况
...
- BigWig部分考虑了测序深度/细胞数影响
  ...
  ```
  scale=$(awk -v n=${total} 'BEGIN{printf "%.10f", 1000000/n}')
  bamCoverage --bam ${out_bam} -o ${out_bw} -of bigwig --binSize 10 --normalizeUsing None --scaleFactor ${scale}
  BigWig值 ≈ 当前bin覆盖reads数 × 1,000,000 / 该celltype×group的总assigned alignments数
  ```
  
  不过这种并没有考虑样本数的影响（reads多/细胞多的样本贡献更大），当然这个其实可以忽略掉，之前snRNA-seq分析时也没看哪个样本中哪类细胞多
