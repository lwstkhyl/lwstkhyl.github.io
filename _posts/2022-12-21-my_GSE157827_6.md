...
![my_GSE157827_6_5](/upload/md-image/other/my_GSE157827_6_5.jpg){:width="500px" height="500px"}

mic的cluster2：
![my_GSE157827_6_6](/upload/md-image/other/my_GSE157827_6_6.jpg){:width="500px" height="500px"}
...
LTR/MER开头的这种，是否应该归入具体某类hERV中：参考stellarscope作者提供的hERV的subfamily-group映射`families.tsv`



那么stellarscope得到的计数是不是已经把LTR归入了具体某类hERV（其family-level的feature只有几个特殊的LTRxxx，而不是像rmsk里面有数百个），毕竟作者说是按照rmsk注释自己合的？看一下stellarscope作者的合成工序？查一下位点是不是一样



#### 把scTE的结果加到已有的seurat对象上

```r
rmsk_filter_repFamily <- read.csv("/public/home/GENE_proc/wth/my_data/rmsk_filter_repFamily.csv")
intersect(herv_features, unique(rmsk_filter_repFamily$repName))
```
