...
write.csv(res_subfamily, file.path(res_dir, "hERV_group_celltype.csv"))
```
...
oligo <- get_subCelltype("Oligo", pc.use = 1:30, harmony = T, resolution = 0.3)
...
```r
oligo$subcluster <- oligo$RNA_snn_res.0.5
seu_list <- list()
...
construct_network <- function(seu, herv_assay, target_herv, mix=FALSE, filter_fraction=0.1, deep_split=4, min_module_size=20, merge_cut_height=0.05){
  celltype_name <- as.character(unique(seu$celltype))
...
construct_network(astro, herv_group_assay, target_herv_group, min_module_size=30, merge_cut_height=0.1)
construct_network(astro, herv_family_assay, target_herv_family, min_module_size=30, merge_cut_height=0.1)
rm(astro)
...
construct_network(excit, herv_group_assay, target_herv_group)
construct_network(excit, herv_family_assay, target_herv_family)
rm(excit)
inhib <- readRDS("/public/home/GENE_proc/wth/my_data/inhib.rds")
construct_network(inhib, herv_group_assay, target_herv_group)
construct_network(inhib, herv_family_assay, target_herv_family)
rm(inhib)
...
construct_network(mic, herv_group_assay, target_herv_group, min_module_size=30, merge_cut_height=0.1)
construct_network(mic, herv_family_assay, target_herv_family, min_module_size=30, merge_cut_height=0.1)
```
