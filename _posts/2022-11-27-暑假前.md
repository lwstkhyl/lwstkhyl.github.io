...
```sh
conda create -n herv_junc samtools regtools bedtools parallel stringtie -y
```

**第一步：STARsolo first-pass，重新生成SJ.out.tab**。这一步主要为了收集所有样本的junction，不以表达矩阵为主要目的
...
**第二步：汇总first-pass junction，构建updated STAR index**。先从first-pass结果里选junction，再把这些junction加入STAR索引用于second-pass
...
**第三步：STARsolo second-pass，生成最终BAM和SJ.out.tab**
...
library(readxl)
library(data.table)
res_dir <- "/public/home/GENE_proc/wth/GSE174367/0530test/res/"
seu <- readRDS("/public/home/GENE_proc/wth/benkebishe/data/all_final.RData")
...
de <- read_xlsx("/public/home/GENE_proc/wth/GSE174367/0530test/res/DE.xlsx", sheet = de_sheet) %>% as.data.frame()
de2 <- de %>%
  mutate(
    locus_raw = feature,
...
candidate_missing <- candidate %>% filter(is.na(chr))
write.csv(candidate_missing, file.path(res_dir, "candidate_hERV_missing_coordinate.csv"), row.names = FALSE)
candidate <- candidate %>%
