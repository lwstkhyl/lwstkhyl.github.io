...
$(document).ready(function () {
    ...
    const getNavHeight = () => {
        ...
        return !header || header.hidden ? 0 : $(header).height() + 1;
    };
    let nav_height = getNavHeight(); //顶部导航栏高度，用于调节滚动距离
    document.addEventListener('header-visibility-change', () => {
        nav_height = getNavHeight();
    });
    window.addEventListener("onload", function () { //加载完成时更新nav_height
        nav_height = getNavHeight();
        setTimeout(() => nav_height = getNavHeight(), 100);
    });
    //目录是否随内容滚动按钮
    const is_scroll = $(`
    ...
    const resize_func = () => {
        nav_height = getNavHeight(); //更新顶部导航栏高度
