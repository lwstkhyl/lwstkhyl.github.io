...
问题：目前真正带有`真实短sgRNA效率`+`target-specific AF3 distance`的数据只有`12个target×20/18/16nt×位置3–10中的可编辑A`=`156条position-level记录`。但是原CRISPRon-ABE的训练规模完全不是这个量级，论文整合后的ABE模型涉及17,941条gRNA，而且原模型还是30nt CNN+多输入+双输出架构
