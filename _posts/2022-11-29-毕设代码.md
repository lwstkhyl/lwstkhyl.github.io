...
meta_map_GSE157827 <- read_xlsx("/public/home/GENE_proc/wth/GSE157827/metadata/metadata.xlsx")
sample_info_GSE157827 <- read_xlsx("/public/home/GENE_proc/wth/GSE157827/metadata/sample_info.xlsx")
...
meta_map_GSE174367 <- read_xlsx("/public/home/GENE_proc/wth/GSE174367/metadata/metadata.xlsx")
sample_info_GSE174367 <- read_xlsx("/public/home/GENE_proc/wth/GSE174367/metadata/sample_info.xlsx")
...
gsm_info <- gsm_info %>%  # 将字符串NA转为真实NA
...
p1 <- VlnPlot(seu, features = c("nFeature_RNA", "nCount_RNA"),
              pt.size = 0, ncol = 2, group.by = "dataset") + NoLegend()
p2 <- VlnPlot(seu, features = c("nFeature_RNA", "nCount_RNA"),
              pt.size = 0, ncol = 2, group.by = "sample_uid") + NoLegend()
p3 <- VlnPlot(seu, features = c("nCount_HERV", "nFeature_HERV"),
              pt.size = 0, ncol = 2, group.by = "dataset") + NoLegend()
p4 <- VlnPlot(seu, features = c("nCount_HERV", "nFeature_HERV"),
              pt.size = 0, ncol = 2, group.by = "sample_uid") + NoLegend()
p5 <- VlnPlot(seu, features = c("percent_mito", "percent_ribo"),
              pt.size = 0, ncol = 2, group.by = "dataset") + NoLegend()
p6 <- VlnPlot(seu, features = c("percent_mito", "percent_ribo"),
              pt.size = 0, ncol = 2, group.by = "sample_uid") + NoLegend()
p7 <- VlnPlot(seu, features = "HERV_fraction",
              pt.size = 0, ncol = 1, group.by = "dataset") + NoLegend()
p8 <- VlnPlot(seu, features = "HERV_fraction",
              pt.size = 0, ncol = 1, group.by = "sample_uid") + NoLegend()
...
cutoff_tbl <- tibble::tribble(
  ...
  "GSE157827",  500,           8000,          1000,        50000,       14,        3,
  "GSE174367",  500,           9000,          1000,        60000,       14,        3
)
...
for (sid in sample_ids) {
  ...
  sce <- SingleCellExperiment(
    assays = list(counts = GetAssayData(sub, assay = "RNA", slot = "counts"))
  )
  sce <- scDblFinder(sce)
...
# 查看双细胞统计结果
table(scRNA_cells$sample_uid, scRNA_cells$doublet_class
pdf(file.path(res_dir, "doublet_score_by_dataset.pdf"), width = 14, height = 6)
print(
  VlnPlot(scRNA_cells, features = "doublet_score", pt.size = 0, group.by = "dataset") + NoLegend()
)
dev.off()
# 过滤
...
```r
p1 <- VlnPlot(seu_filt, features = c("nFeature_RNA", "nCount_RNA"),
              pt.size = 0, ncol = 2, group.by = "dataset") + NoLegend()
p2 <- VlnPlot(seu_filt, features = c("nFeature_RNA", "nCount_RNA"),
              pt.size = 0, ncol = 2, group.by = "sample_uid") + NoLegend()
p3 <- VlnPlot(seu_filt, features = c("nCount_HERV", "nFeature_HERV"),
              pt.size = 0, ncol = 2, group.by = "dataset") + NoLegend()
p4 <- VlnPlot(seu_filt, features = c("nCount_HERV", "nFeature_HERV"),
              pt.size = 0, ncol = 2, group.by = "sample_uid") + NoLegend()
p5 <- VlnPlot(seu_filt, features = c("percent_mito", "percent_ribo"),
              pt.size = 0, ncol = 2, group.by = "dataset") + NoLegend()
p6 <- VlnPlot(seu_filt, features = c("percent_mito", "percent_ribo"),
              pt.size = 0, ncol = 2, group.by = "sample_uid") + NoLegend()
...
```r
seu <- NormalizeData(seu, verbose = FALSE)  # 计算RNA assay的data
...
seu <- RunUMAP(seu, reduction = "harmony", dims = pc.use, verbose = FALSE)
p_dim_group <- DimPlot(seu, group.by = "group")
p_dim_dataset <- DimPlot(seu, group.by = "orig.ident", raster = TRUE)
# 看一下效果
DimPlot(seu, group.by = "orig.ident", raster = TRUE)
p_dim_group
p_dim_dataset
# 再画clustree
seu <- FindNeighbors(seu, reduction = "harmony", dims = pc.use, verbose = FALSE)
seu <- FindClusters(seu, resolution = c(0.01, 0.05, 0.1, 0.2, 0.5))
...
save(all_markers_sig, file = "/public/home/GENE_proc/wth/my_data/all_all_markers_sig.RData")
```
