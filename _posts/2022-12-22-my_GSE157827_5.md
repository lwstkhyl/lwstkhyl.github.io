...
- 先看这个亚群的模块表达分数（就是上面得到的几个关键模块）
- 把这个亚群和其它亚群做差异表达分析，得到一组亚群特异上调基因，看这些基因集中在哪个模块，综合模块表达分数，确定这个亚群在哪个模块上特异表达
...
for (cl in target_clusters) {
  ...
  deg_up <- subset(deg, p_val_adj < 0.05 & deg[[fc_col]] > fc_cut)
...
综合上面的结果，HERVH-int和HERVK11-int的GO结果最显著，且它们都在blue这个module，对该模块的GO分析结果也显著集中于蛋白翻译+线粒体能量代谢。另外几个都在turquoise模块，不过这个模块的GO结果于方案1的类似，都比较模糊，所以就以HERVH-int和HERVK11-int为主要分析对象看一下
- 一个比较大的问题就是与hERV相关的前300个基因的相关性在0.05±0.02左右，这个相关性应如何衡量？
- blue模块与上面方案1的blue应该是类似的模块，但方案1中这个模块和hERVK是负相关的，然而把hERV放进网络中，却把一些hERV也归入了这个模块（相当于与这个模块正相关）
