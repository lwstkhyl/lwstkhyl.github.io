...
conda create -n leafcutter_herv -c davidaknowles leafcutter r-leafcutter -y
conda create -n r-tracklayer r-base r-data.table r-stringr r-openxlsx bioconductor-genomicranges bioconductor-rtracklayer -y
```
...
fwrite(meta[, .(sample_uid, bam, group, brain_region, SRR, GSM, AGE, sex, pmi)], file.path(res_dir, "leafcutter_bams.tsv"), sep = "\t")
for (br in unique(meta$brain_region)) {
  x <- meta[brain_region == br]
  if (length(unique(x$group)) < 2) next
  fwrite(x[, .(sample_uid, group)], file.path(res_dir, paste0("group_", br, ".txt")), sep = "\t", col.names = FALSE)
...
# find "${CONDA_PREFIX}" -name "leafcutter_cluster_regtools.py"
...
# find "${CONDA_PREFIX}" -name "leafcutter_ds.R"
ds_r=$(which leafcutter_ds.R || true)
module load miniconda3/base
conda activate regtools
tail -n +2 "${res_dir}/leafcutter_bams.tsv" | while IFS=$'\t' read -r sample_uid bam group brain_region SRR GSM AGE sex pmi; do
  out="${junc_dir}/${sample_uid}.junc"
  if [[ ! -s "${out}" ]]; then
    echo "[regtools] ${sample_uid}"
    regtools junctions extract -a 8 -m 50 -M 500000 -o "${out}" "${bam}"
  fi
done
...
for group_file in "${res_dir}"/group_*.txt; do
  ...
  mkdir -p "${out_region}"
  awk -v junc_dir="${junc_dir}" '{print junc_dir"/"$1".junc"}' "${group_file}" > "${res_dir}/junc_list_${br}.txt"
  ...
  python "${cluster_py}" \
    ...
    -o "LC_${br}" \
    -r "${out_region}"
  counts_file=$(ls "${out_region}"/LC_${br}*perind*.counts.gz | head -n 1)
  if [[ -z "${counts_file}" ]]; then
    echo "找不到LeafCutter counts文件：${br}"
    exit 1
  fi
  echo "[LeafCutter differential splicing] ${br}"
  Rscript "${ds_r}" \
    --num_threads "${threads}" \
    "${counts_file}" \
    "${group_file}" \
    "${out_region}/LC_${br}_AD_vs_Control"
done
```
